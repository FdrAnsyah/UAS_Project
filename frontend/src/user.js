...
=======
  ...
  function displayEmployees() {
    ...
    employees.forEach(emp => {
      ...
      `;
      tbody.appendChild(tr);
>>>>>>> c8635e16ace6060852714a15c3ba9e19adea471c
    });
  }
...
const addPasien = (e) => {
  ...
  const pasien = {
    ...
    goldarah: document.getElementById('goldarah').value,
    rhesus: document.getElementById('rhesus').value,
    alergi: document.getElementById('alergi').value.split(',').map(item => item.trim()),
    penyakitBawaan: document.getElementById('penyakitBawaan').value.split(',').map(item => item.trim()),
...
const displayPasien = () => {
  ...
  // Apply filters
  datapasien = datapasien.filter(pasien => {
    const matchSearch = pasien.nama.toLowerCase().includes(searchTerm) ||
  ...
  datapasien.forEach(pasien => {
    ...
    const getStatusColor = (status) => {
      switch(status) {
        case 'Rawat Inap': return 'bg-yellow-100 text-yellow-800';
        case 'Rawat Jalan': return 'bg-green-100 text-green-800';
        case 'IGD': return 'bg-red-100 text-red-800';
...
const showRiwayat = (id) => {
  ...
  if (pasien && pasien.riwayatKunjungan.length > 0) {
    alert(`Riwayat kunjungan ${pasien.nama}:\n\n${
      pasien.riwayatKunjungan
        .map(r => `${formatDate(r.tanggal)}: ${r.keterangan}`)
        .join('\n')
    }`);
  } else {
    alert('Belum ada riwayat kunjungan');
